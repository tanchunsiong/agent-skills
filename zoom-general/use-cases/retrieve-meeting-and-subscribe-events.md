...
**You cannot dynamically subscribe to zoom-webhooks/WebSockets for a single meeting**. These are configured at app creation time in the Marketplace portal and receive events for ALL meetings in the account. Your code filters events by meeting ID.
...
│                                                                 │
│ Step 3: POST /zoom-webhooks/zoom (webhooks)                    │
...
async function getAccessToken() {
  ...
  const response = await axios.post(
    `https://zoom.us/zoom-oauth/token?grant_type=account_credentials&account_id=${process.env.ZOOM_ACCOUNT_ID}`,
    null,
...
app.post('/zoom-webhooks/zoom', async (req, res) => {
...
app.listen(3000, () => {
  ...
  console.log('  POST /zoom-webhooks/zoom     - Webhook receiver');
});
```
...
**See comprehensive implementation:** [zoom-rtms/examples/rtms-bot.md](../../zoom-rtms/examples/rtms-bot.md)
...
// Step 2: Wait for meeting.rtms_started webhook
app.post('/zoom-webhooks/zoom', (req, res) => {
  const { event, payload } = req.body;
...
- **WebSockets**: https://developers.zoom.us/docs/api/zoom-websockets/
- **Meeting SDK**: https://developers.zoom.us/docs/zoom-meeting-sdk/web/
- **RTMS**: https://developers.zoom.us/docs/zoom-rtms/
