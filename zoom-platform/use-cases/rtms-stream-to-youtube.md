...
Stream Zoom meeting audio and video to YouTube Live via RTMS and FFmpeg.

## Overview

This guide demonstrates how to capture audio and video from Zoom RTMS and re-stream them to YouTube Live using FFmpeg's RTMP output. The RTMS media WebSocket delivers raw PCM audio (L16, 16kHz, mono) and H.264 video, which are piped into FFmpeg for re-encoding and pushed to YouTube's RTMP ingest endpoint. A gap-filling strategy injects silent audio and black video keyframes when participants mute, preventing FFmpeg stream stalls.

## Prerequisites

- YouTube channel with live streaming enabled and a Stream Key
...
// Start FFmpeg process that pipes to YouTube RTMP
function startYouTubeStream() {
  const ffmpeg = spawn('ffmpeg', [
    '-framerate', '25', '-f', 'h264', '-i', 'pipe:3',       // H.264 video input
    '-f', 's16le', '-ar', '16000', '-ac', '1', '-i', 'pipe:4', // PCM audio input
    '-filter_complex',
...
function connectMedia(mediaUrl, meetingUuid, streamId, signalingWs) {
  ...
  mediaWs.on('message', (data) => {
    ...
    if (msg.msg_type === 14 && msg.content?.data) {
      audioStream.write(Buffer.from(msg.content.data, 'base64'));
    }
    if (msg.msg_type === 15 && msg.content?.data) {
      videoStream.write(Buffer.from(msg.content.data, 'base64'));
    }
  });
}
...
```python
import os, json, hmac, hashlib, random, threading, subprocess
...
def start_youtube_stream():
    """Launch FFmpeg to transcode RTMS media to YouTube RTMP."""
    cmd = [
        'ffmpeg',
...
def start_rtms_flow(meeting_uuid, stream_id, server_url):
    ...
    ffmpeg = start_youtube_stream()

    # Connect signaling, then media, pipe audio/video to FFmpeg
    # (Follow the two-phase WebSocket pattern from RTMS_CONNECTION_FLOW.md)
    # On msg_type 14 (audio): ffmpeg.stdin.write(base64.b64decode(content['data']))
    # On msg_type 15 (video): ffmpeg.stdin.write(base64.b64decode(content['data']))
    pass
...
- [RTMS YouTube Streaming Sample](https://github.com/zoom/rtms-samples/tree/main/streaming/stream_audio_and_video_to_youtube_greedy_gap_filler_js)
